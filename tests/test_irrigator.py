...
class TestWateringDecision:
    ...
    def test_partial_forecast_reduces_duration(self, low_bucket):
        ...
        no_forecast = WeatherData(
            18.0, 32.0, precipitation_mm=0.0, forecast_precip_mm=0.0
        )
        some_forecast = WeatherData(
            18.0, 32.0, precipitation_mm=0.0, forecast_precip_mm=5.0
        )
...
class TestConversions:
    ...
    def test_larger_area_means_more_volume_same_duration_ratio(self):
        ...
        zone_small = ZoneConfig(
            ["tomato"], PLANTING, zone_area_m2=10.0, flow_rate_lpm=5.0
        )
        zone_large = ZoneConfig(
            ["tomato"], PLANTING, zone_area_m2=20.0, flow_rate_lpm=5.0
        )
    ...
    def test_higher_flow_rate_means_shorter_duration(self):
        zone_slow = ZoneConfig(
            ["tomato"], PLANTING, zone_area_m2=10.0, flow_rate_lpm=2.0
        )
        zone_fast = ZoneConfig(
            ["tomato"], PLANTING, zone_area_m2=10.0, flow_rate_lpm=8.0
        )
        weather = WeatherData(18.0, 32.0, precipitation_mm=0.0, forecast_precip_mm=0.0)
...
class TestSeasonalVariation:
    def test_kc_lower_at_seedling_than_mid_season(self):
        ...
        r_seedling = calculate(
            weather,
            ZoneConfig(["tomato"], planting, 10.0, 5.0),
            b1,
            date(2024, 4, 6),
            MILAN_LAT,
        )  #  5 days
        r_midseason = calculate(
            weather,
            ZoneConfig(["tomato"], planting, 10.0, 5.0),
            b2,
            date(2024, 7, 1),
